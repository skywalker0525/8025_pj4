...
\begin{abstract}
Construction inspection is still heavily dependent on manual site walks, visual judgement, and fragmented records. Mobile robots can make inspection more repeatable, but construction sites are cluttered, changing, and safety-critical, so an autonomous robot should first be evaluated in a controlled digital environment. This paper presents a ROS 2-based construction robot digital twin prototype for an indoor inspection task. The system combines Gazebo simulation, a four-wheel mobile robot model, LiDAR and RGB sensing, Nav2 autonomous navigation, SLAM-ready mapping, mission orchestration, and a browser-based FastAPI dashboard. The implemented task is to start from point A, navigate through a site-like room, approach a wall-mounted AprilTag target at point B, and report mission state and telemetry to an operator. The repository was reviewed at the package and node level to document the implemented architecture rather than only the demonstration outcome. The prototype shows how simulation, navigation, telemetry, and human supervision can be connected into an executable construction robot twin. It also exposes clear limits: the AprilTag is currently a known map target rather than a detected visual pose, the platform is a mobile inspection robot rather than a mobile manipulator, and the work has not been deployed on physical hardware.
\end{abstract}
...
This project implements that idea at prototype scale. The repository is available at \url{https://github.com/skywalker0525/8025_pj4}. It contains a ROS 2 workspace with separate packages for robot description, simulation, navigation, mission logic, and a local web dashboard. The robot is a four-wheel mobile inspection platform with LiDAR, camera, odometry, transforms, and a pan/tilt gimbal. The site is a Gazebo room with walls, material-like obstacles, barriers, a tool chest, an inspection column, and a wall-mounted AprilTag. The task is deliberately narrow: the robot starts at point A, navigates to the AprilTag approach point B, and reports the mission as complete.

The contribution of this paper is not a new navigation algorithm. It is an integrated, reproducible digital twin architecture for construction robot inspection. The paper strengthens the construction background, reviews related research, documents the implemented codebase, and discusses what the prototype can and cannot claim. The report is written honestly: although the CIVIL 8025 Project 4 direction mentions mobile manipulation and MoveIt 2 as possible elements, the submitted repository implements a mobile inspection robot twin and does not implement an arm, MoveIt 2 planning, or real AprilTag pose estimation.
...
\begin{figure}[htbp]
  ...
  \end{tikzpicture}}
  \caption{Online digital twin literature commonly frames the twin as a closed loop from asset state and sensing to model-based reasoning and operational decisions; this figure is redrawn as a synthesis from \citet{boje2020semantic}, \citet{jiang2021digital}, and \citet{madubuike2022review}.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \foreach \n in {mobility,perception,localization,safety,interface,records}
      \draw[arrow] (\n) -- (robot);
  \end{tikzpicture}}
  \caption{Construction-robotics studies identify a recurring set of requirements around mobility, perception, navigation, safety, interfaces, and data records; this figure is redrawn from the themes reviewed by \citet{delgado2019robotics} and \citet{asadi2018vision}.}
...
\begin{table}[htbp]
  \centering
  \caption{Implemented modules and evidence found in the repository.}
  \label{tab:modules}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Navigation evidence used by the digital twin. The 2D obstacles follow the repository's Gazebo world poses, and the trajectory shows the recorded A--B mission path.}
  \label{fig:route-map}
\end{figure}
...
Future work should therefore focus on closing the loop between visual perception and navigation. AprilTag detection could estimate the final tag pose and support a short visual-servoing approach. The dashboard could display a simplified map, scan, and costmap overlay. A manipulator extension could add MoveIt 2 planning with construction obstacles represented as collision objects. Finally, physical deployment would require calibration, sensor noise handling, emergency stop hardware, and safety procedures for human-robot coexistence.
